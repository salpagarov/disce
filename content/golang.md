...
"Маленькие книги" Карла Сегуина – лучшие в жанре "быстрого старта". В них ничего лишнего, только самое главное – подробности, в которых так легко утонуть поначалу, потом так же легко восполняются правильными вопросами и самостоятельно найденными ответами. 
...
Это официальное название. Одна-единственная "команда go" используется для компиляции, выполнения, тестирования, управления зависимостями, форматирования исходного кода… для всего.  

Настоящий "швейцарский нож", осваивать который лучше постепенно.
...
Любая программа на Go состоит из модулей. Одни входят в стандартную библиотеку, другие ваши собственные, третьи написаны кем-то еще. Некоторым модулям для работы нужны другие модули, тем, в свою очередь, еще какие-то и все это в конечном итоге образует дерево зависимостей. Если модуль на “вершине” этого дерева называтся `main` то после сборки вы получите исполнимый файл, в котором будет и этот модуль, и все его зависимости. В другом случае модуль будет просто еще одним “кубиком” для построения других программ.
